...
<article class="container">
	<div class="col-md-12">
		...
		<form name="frm" id="frm" class="form-signin" method="post">
			...
			<button class="btn btn-lg btn-primary btn-block" type="button" id="findPwBtn">비밀번호 찾기</button>
			<br><br>
		</form><br><br>
		<hr>
	</div>
		...
		$('#findPwBtn').on('click', function(){
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(data.error){
						$('#MsgId').html('<p style="color:red">존재하지않는 아이디입니다.</p>');
						$("#id").focus();
						return false;
					}else{
						$('#MsgId').html('<p></p>');
